...
def AB_PPAAC(train_name, train_label, test_name, test_label, PAAC_Feature):
    ...
    for second_i in range(train_label.shape[0]):
        ...
        if train_label[second_i] == "3":  # Negtive
            pair.append(0)
        for second_i3 in range(len(PAAC_Feature)):
            if train_name[second_i] == PAAC_Feature[second_i3][0].split("|")[1]:
                pair.extend(PAAC_Feature[second_i3][1:])
                break
    ...
    for second_i in range(test_label.shape[0]):
        ...
        if test_label[second_i] == "3":
            pair.append(0)
        for second_i3 in range(len(PAAC_Feature)):
            if test_name[second_i] == PAAC_Feature[second_i3][0].split("|")[1]:
                pair.extend(PAAC_Feature[second_i3][1:])
                break
